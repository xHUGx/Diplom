...
from utils.MyOwnClass import Swimmer
...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        cv2.line(im0s, (160, 845), (165, 920),(0,255,0), 2)
        cv2.line(im0s, (1610, 625), (1660, 680),(0,255,0), 2) 
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                for *xyxy, pid in track_bbs_ids:
                    cv2.putText(im0, str(s), (800,50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA) 
                    #Pool Speed############################## 
                    cv2.putText(im0, str(s), (800,50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,0), 2, cv2.LINE_AA)   
                    if ((xyxy[2]+xyxy[0])/2>1610) and ((xyxy[3]+xyxy[1])/2>625) and ((xyxy[3]+xyxy[1])/2<700):
                        swimmer1.mark1 = monotonic()
                        swimmer1.check1 = True

                    if ((xyxy[0]+xyxy[2])/2<165) and ((xyxy[1]+xyxy[3])/2>820) and ((xyxy[3]+xyxy[1])/2<920):                 
                        swimmer1.mark2 = monotonic()
                        swimmer1.check2 = True

                    if (swimmer1.check1==True)and(swimmer1.check2==True):                                          
                        swimmer1.check1=False
                        swimmer1.check2=False
                        swimmer1.av_speed = swimmer1.speed_measure(swimmer1.mark1,swimmer1.mark2)                     
                    ...
                    if save_img or view_img:  # Add bbox to image
                        ...
                        #cv2.putText(im0, str(label), (800,200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA)
                        #cv2.putText(im0, str(len(trajectory[label])), (800,250), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA)
                        for i in range(1,len(trajectory[label])):                           
                            ...
                            if (i%5==0):
                                time_mark[label] = monotonic()
                                speed[label] = ((math.sqrt((trajectory[label][i][0] - trajectory[label][i-5][0])**2 + (trajectory[label][i][1] - trajectory[label][i-5][1])**2))/66)/0.3                                
                                k=k+1
                        
                        if speed[label] is not None and i>5 and label is not None:                                                     
                            cv2.putText(im0, f' {speed[label]:.2f} m/s', (trajectory[label][i]), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA) 
                            file.write(f'label: {label} speed:{speed[label]:.2f}\n')       
                            print(time_mark[label])                                                                
                        #cv2.putText(im0, str(k), (800,150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA)       
                            #cv2.putText(im0, str(speed[label]), (800,550), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA)      
                            #cv2.putText(im0, str(trajectory[label][i][0]), (800,550), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA)       
                        ...
                        att_count = -1
                        #if (speed[label])is not None and :
                            #draw.text((x1, y1-50),str(speed[label]), (205, 55, 0), font=font)  # coordinate，text，color，typeface
            ...
            cv2.putText(im0, f'av_speed:{swimmer1.av_speed:.2f} m/s', (50,50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA)     
            cv2.putText(im0, "Debug:", (50,100), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,0), 2, cv2.LINE_AA)  
            cv2.putText(im0, f'prev_av_speed:{swimmer1.prev_av_speed:.2f} m/s', (50,150), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA)  
            cv2.putText(im0, f'mark1:{swimmer1.mark1:.2f} sec', (50,200), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA)     
            cv2.putText(im0, f'mark2:{swimmer1.mark2:.2f} sec', (50,250), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA)     
            cv2.putText(im0, f'check1:{swimmer1.check1}', (50,300), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA)    
            cv2.putText(im0, f'check2:{swimmer1.check2}', (50,350), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2, cv2.LINE_AA)        
